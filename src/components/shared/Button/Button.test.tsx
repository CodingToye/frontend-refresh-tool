...
describe("Button", () => {
  ...
  it("renders an icon when buttonIcon is provided", () => {
    render(
      <Button buttonLabel="Save" buttonIcon="check" handleClick={() => {}} />,
    );
  ...
  it("does not render an icon element when buttonIcon is omitted", () => {
    ...
    expect(
      screen.queryByRole("button")!.querySelector(".material-symbols-outlined"),
    ).toBeNull();
  });
  ...
  it("applies tertiary style classes when buttonStyle is tertiary", () => {
    ...
    expect(screen.getByRole("button")).toHaveClass("bg-tertiary-500");
  });
